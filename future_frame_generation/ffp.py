...
def parse_args():
    ...
    if args.instance_data_dir is None:
        raise ValueError("You must specify a target frame directory.")

    if args.cond_data_dir is None:
        raise ValueError("You must specify a previous frame directory.")

    if args.cond_data_dir2 is None:
        raise ValueError("You must specify a previous frame directory2.")
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.cond_data_root = Path(cond_data_root)
        if not self.cond_data_root.exists():
        ...
        self.cond_images_path2 = list(Path(cond_data_root2).iterdir())
    ...
    def __getitem__(self, index):
        ...
        cond_image = Image.open(self.cond_images_path[index % self.num_instance_images])
        cond_image2 = Image.open(self.cond_images_path2[index % self.num_instance_images])
...
def main():
    ...
    def collate_fn(examples):
        ...
        cond_pixel_values2 = [example["cond_images2"] for example in examples]
        ...
        cond_pixel_values2 = torch.stack(cond_pixel_values2)
        cond_pixel_values2 = cond_pixel_values2.to(memory_format=torch.contiguous_format).float()
